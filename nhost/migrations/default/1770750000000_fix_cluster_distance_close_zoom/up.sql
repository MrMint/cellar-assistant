-- Fix cluster distance calculation at close zoom levels
--
-- Problem: At max zoom (zoom 22), viewport is ~50m wide, but the function has
-- a hardcoded 50m minimum cluster distance, so it clusters nearly everything
-- visible on screen. This makes it impossible to see individual places at
-- close zoom.
--
-- Fix 1: Skip clustering entirely when viewport < 500m across
-- Fix 2: Make cluster distance floor proportional to viewport size instead of
--        a fixed 50m (use viewport_max_dim_km * 10 instead of 50)
...
BEGIN
  ...
  -- Floor changed from fixed 50m to viewport_max_dim_km * 10 to prevent
  -- clustering at very close zoom levels (e.g., zoom 22 where viewport is ~50m)
  -- ============================================================
  density_factor := LEAST(1.0, SQRT(bounded_count::float / GREATEST(result_limit, 1)::float));
  cluster_distance_meters := GREATEST(viewport_max_dim_km * 10, LEAST(500000, viewport_max_dim_km * 50 * density_factor));

  -- Always cluster if we have more than 20 places AND viewport >= 500m
  -- Skip clustering at very close zoom (viewport < 500m) to show individual places
  IF bounded_count > 20 AND viewport_max_dim_km >= 0.5 THEN
