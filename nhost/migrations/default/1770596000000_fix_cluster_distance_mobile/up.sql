-- Fix cluster distance calculation for narrow (mobile) viewports
--
-- Problem: SQRT(viewport_area_km2) penalizes narrow viewports. On mobile,
-- viewport is tall and narrow (~33% the width of desktop), so area is ~33%
-- of desktop. This produces a ~57% cluster distance, meaning fewer points
-- merge into clusters, and mobile ends up with MORE clusters on a SMALLER screen.
--
-- Fix: Use viewport's max dimension (in km) instead of square root of area.
-- For square viewports, SQRT(width * height) ≈ max(width, height) so behavior
-- is identical. For narrow viewports, max dimension gives a larger, more
-- appropriate cluster distance.
