-- Restore both map search functions to previous versions without tier_list_ids parameter
...
BEGIN
  ...
  -- This is key for handling narrow mobile viewports correctly:
  -- instead of SQRT(area) which penalizes narrow viewports, we use
  -- the larger of width or height. For square viewports, the two
  -- formulas are equivalent. For narrow viewports, max dimension
  -- produces appropriately larger cluster distances.
  ...
  -- Uses viewport_max_dim_km instead of SQRT(viewport_area_km2_calc)
  -- to handle narrow mobile viewports correctly. For square viewports,
  -- the two formulas are equivalent. For narrow viewports, max dimension
  -- produces appropriately larger cluster distances, preventing the
  -- over-clustering that occurs with area-based calculation.
...
-- ============================================================
-- 2. search_places_hybrid (semantic search) — restore websearch_to_tsquery version without tier_list_ids
-- ============================================================
...
BEGIN
  ...
  text_matches AS (
    SELECT bp.id, ts_rank_cd(bp.search_text, tsquery_val, 32) AS rank_score
    FROM bounded_places bp
    WHERE has_text_query AND bp.search_text @@ tsquery_val
    ORDER BY rank_score DESC LIMIT result_limit * 3
  ),
  trgm_matches AS (
    SELECT bp.id, similarity(bp.name, search_query) AS sim_score
    FROM bounded_places bp
    WHERE has_text_query AND similarity(bp.name, search_query) >= 0.15
    ORDER BY sim_score DESC LIMIT result_limit * 2
  ),
  category_matches AS (
    SELECT bp.id,
      LEAST(1.0, scores.cat_score * CASE WHEN bp.primary_category = ANY(matched_categories) THEN 1.15 ELSE 1.0 END) AS cat_score
    FROM bounded_places bp
    CROSS JOIN LATERAL (
      SELECT COALESCE(MAX(CASE
        WHEN category_scores IS NOT NULL AND array_position(matched_categories, cat) IS NOT NULL
        THEN category_scores[array_position(matched_categories, cat)] ELSE 0.5
      END), 0) AS cat_score
      FROM unnest(bp.categories) AS cat WHERE cat = ANY(matched_categories)
    ) scores
    WHERE has_categories AND (bp.primary_category = ANY(matched_categories) OR bp.categories && matched_categories)
  ...
  merged AS (
    SELECT COALESCE(tm.id, tg.id, cm.id) AS place_id,
      ...
      (COALESCE(tm.rank_score, 0.0) * 0.35 + COALESCE(tg.sim_score, 0.0) * 0.25 + COALESCE(cm.cat_score, 0.0) * 0.40)::FLOAT8 AS combined_score
  ...
  deduped AS (
    SELECT DISTINCT ON (place_id) place_id, text_rank, trigram_similarity, category_score, combined_score
    FROM merged ORDER BY place_id, combined_score DESC
  )
