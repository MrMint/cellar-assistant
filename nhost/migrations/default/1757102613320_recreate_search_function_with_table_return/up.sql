-- Create the function with TABLE return type using text for geometry fields
...
BEGIN
  ...
  IF total_places > 20 THEN
    ...
    SELECT 
      ...
      ST_AsGeoJSON(cs.center_point)::text,
      ST_AsGeoJSON(cs.bounds)::text,
      gen_random_uuid(),
    ...
    SELECT 
      ...
      NULL::text, NULL::text,
      sp.id, sp.name, ST_AsGeoJSON(sp.location)::text,
  ...
  ELSE
    ...
    SELECT 
      ...
      NULL::integer, NULL::integer,
      NULL::text, NULL::text,
      p.id, p.name, ST_AsGeoJSON(p.location)::text,
