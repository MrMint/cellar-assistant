-- Normalize text_rank to 0-1 range for proper weight contribution
--
-- Problem: ts_rank_cd() returns values in ~0.001–0.15 range, while trigram_similarity
-- and category_score are already 0–1. This means the 35% weight on text_rank contributes
-- almost nothing to the combined_score.
--
-- Solution: Normalize text_rank within the result set so the best text match gets 1.0
-- (and therefore the full 0.35 weight).
