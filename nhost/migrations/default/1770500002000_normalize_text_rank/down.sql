-- Restore the function from the websearch_tsquery migration
-- Switch search_places_hybrid from plainto_tsquery to websearch_to_tsquery
--
-- plainto_tsquery('simple', 'wine bar downtown') => 'wine' & 'bar' & 'downtown'
--   All terms required (implicit AND), no support for OR / phrases / negation.
--
-- websearch_to_tsquery('simple', 'wine bar downtown') => 'wine' & 'bar' & 'downtown'
--   Same default behavior for simple queries, but also supports:
--     - OR:       'wine OR beer'        => 'wine' | 'beer'
--     - Phrases:  '"craft beer"'        => 'craft' <-> 'beer'
--     - Negate:   '-chain'              => !'chain'
--
-- This is a drop-in improvement: existing queries produce identical tsquery output,
-- but users who type OR, quotes, or minus signs get meaningful results instead of
-- broken matches.
