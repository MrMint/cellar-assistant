-- Create new function that returns GeoJSON text for geometry fields
CREATE OR REPLACE FUNCTION public.search_places_adaptive_cluster_geojson(
...
RETURNS SETOF place_search_results_new
LANGUAGE plpgsql
STABLE
AS $function$
DECLARE
...
BEGIN
  ...
  IF total_places > 20 THEN
    ...
    SELECT 
      ...
      ST_AsGeoJSON(cs.center_point)::text, -- Convert to GeoJSON text
      ST_AsGeoJSON(cs.bounds)::text,      -- Convert to GeoJSON text
      gen_random_uuid(),
    ...
    SELECT 
      ...
      NULL::text, NULL::text, -- NULL text instead of NULL geography/geometry
      sp.id, sp.name, ST_AsGeoJSON(sp.location)::text, -- Convert to GeoJSON text
  ...
  ELSE
    ...
    SELECT 
      ...
      NULL::integer, NULL::integer,
      NULL::text, NULL::text, -- NULL text instead of NULL geography/geometry
      p.id, p.name, ST_AsGeoJSON(p.location)::text, -- Convert to GeoJSON text
