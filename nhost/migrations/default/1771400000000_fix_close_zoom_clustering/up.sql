-- Fix: clusters persisting at close zoom levels
--
-- Two root causes:
-- 1. GREATEST(50, ...) enforced a 50-meter minimum cluster distance.
--    At street level (~1km viewport), the natural computed distance
--    (~15m) was bumped to 50m — half a city block — grouping places
--    that should be separate markers.
-- 2. The clustering threshold (bounded_count > 20) was zoom-independent.
--    At street level even 30-50 places should show individually.
--
-- Fixes applied to BOTH tier-list fast path and standard path:
--   a) Replace fixed 50m floor with viewport-proportional minimum:
--      viewport_max_dim_km * 5  (≈ 0.5% of viewport width in meters)
--      → 1km viewport ⇒ 5m min   (only overlapping markers cluster)
--      → 10km viewport ⇒ 50m min (same as before)
--      → 100km viewport ⇒ 500m min
--   b) Add zoom-dependent clustering threshold:
--      < 2km viewport ⇒ skip clustering entirely (show individuals)
--      2–5km viewport ⇒ cluster only when > 100 places
--      > 5km viewport ⇒ cluster when > 20 (unchanged)
